...
class Command(BaseCommand):
    ...
    def build_counts(self, tweets, output=None): 
        writer = csv.writer(output, delimiter="\t") if output is not None else None
        if writer is not None:
            writer.writerow(['id', 'user_id', 'text', 'issues'])

        counts = Counter()
        print("Building counts by issue")
        for id, user_id, text in tweets:
            issues = identify_issues(text.lower())
            if writer is not None:
                writer.writerow([id, user_id, text, ",".join(issues.keys())])
    ...
    def handle(self, *args, **options):
        if options['input'] is not None:
            with open(options['input']) as f:
                ...
                self.build_counts(tweets, options['output'] if options['write_output'] is not None else None)
        else:
            tweets = map(lambda t: (t.id, t.user_id, t.text), Tweet.objects.all())
            self.build_counts(tweets, options['output'] if options['write_output'] is not None else None)
